...
phyloseq_results <- function(phyloseq_obj, rank, transformation, distance_measure_results, class_variable, phasetxt)
{

    results_name <- paste(phasetxt, taxa_rank, transformation, distance_measure_results, sep='_')
    scene <- list(camera = list(eye = list(x=1.5, y=1.5, z = 1.5)))
    pcoa_path <- file.path(results_path, paste(paste('PCOA', results_name, sep='_'), 'png', sep='.'))
    ...
    if(!(missing(rank))) {
        phyloseq_obj <- phyloseq_aggregation(phyloseq_obj, rank)
    }

    if(!(missing(transformation))) {
        phyloseq_obj <- phyloseq_transformation(phyloseq_obj, transformation)
    }
    ...
    phyloseq_obj.pcoa <- vegan::metaMDS(phyloseq_obj.distance, k = 3, try = 200, trymax = 200)

    phyloseq_obj.pcoa <- phyloseq_obj.pcoa$points %>% data.frame() %>% rownames_to_column("sample")
    colnames(phyloseq_obj.pcoa) <- c('sample', 'MDS1', 'MDS2', 'MDS3')
    phyloseq_obj.pcoa <- phyloseq_obj.pcoa %>% left_join(phyloseq_obj.data, by = "sample")

    # result1 <- beta_diversity_3d(phyloseq_obj.pcoa, sample_data(phyloseq_obj), class_variable, marker = list(size = 3))
    result1 <- plot_ly(phyloseq_obj.pcoa, x=~MDS1,y=~MDS2,z=~MDS3) %>% add_markers(color=phyloseq_obj.pcoa[[class_variable]], colors = c("darkgreen", "red"), size=3)
    result1 <- result1  %>% layout(scene = scene)
    plotly_IMAGE(result1, format = "png", out_file = pcoa_path, scale = 2)
    ...
    colnames(phyloseq_obj.tsne) <- c('tSNE1', 'tSNE2', 'tSNE3', 'variable')

    result2 <- plot_ly(phyloseq_obj.tsne, x=~tSNE1,y=~tSNE2,z=~tSNE3) %>% add_markers(color=phyloseq_obj.tsne[[class_variable]], colors = c("darkgreen", "red"), size=3)
    ...
    if (nrow(t(otu_table(phyloseq_obj))) < 15){
        ...
        umap <- umap(t(otu_table(phyloseq_obj)), n_components=3, n_neighbors=nneighbors, metric='braycurtis')
    }else{
        umap <- umap(t(otu_table(phyloseq_obj)), n_components=3, metric='braycurtis', random_state=39)
    }
    ...
    result3 <- plot_ly(phyloseq_obj.umapdata, x=~UMAP1,y=~UMAP2,z=~UMAP3) %>% add_markers(color=phyloseq_obj.umapdata[[class_variable]], colors = c("darkgreen", "red"), size=3)
...
spec <- matrix(c(
  ...
  'taxa_rank'            , 'r', 1, "character", "TAXA RANK AGGREGATION (phylum, class, order, family, genus, species)",
  'transformation'       , 'a', 1, "character", "OTU ABUNDANCE TRANSFORMATION (counts, relative, clr (Centered Log Ratio), Log10)",
  'distance_measure'     , 'u', 1, "character", "DISTANCE TO USE FOR ML ANALYSIS (gunifrac, unifrac, wunifrac, va-wunifrac)",
  'distance_measure_results', 'd', 1, "character", "DISTANCE TO USE FOR RESULTS (bray, jaccard, unifrac, wunifrac, dpcoa, minkowski, jsd)",
...
|| is.null(opt$taxa_rank) || is.null(opt$transformation) || is.null(opt$distance_measure) || is.null(opt$distance_measure_results)
...
if (phase1 == TRUE) {
    ...
    # CLASS 2
    study.class2.distances <- dist_get(dist_calc(study.class1, dist=distance_measure))
    ...
    # phyloseq_results(study.phase1, rank, transformation, distance_measure_results, class_variable, 'phase1')
    ...
    for(i in 1:nrow(col_df)) {
        ...
        # phyloseq_results(study.phase1, rank, transformation, distance_measure_results, class_variable, paste(file_name, 'phase2', sep='_'))
...
#
#
# sample.prop.genus <- tax_glom(sample.prop, taxrank = "Genus")
# tax_table(sample.prop.genus) %>% gsub("g__", "", .) %>% gsub("k__", "", .) %>% gsub("p__", "", .) %>% gsub("c__", "", .) %>% gsub("o__", "", .) %>% gsub("f__", "", .) -> tax_table(sample.prop.genus)
#
# sample.metacoder <- parse_phyloseq(sample.prop.genus)
#
# sample.metacoder$data$otu_abund <- calc_taxon_abund(sample.metacoder, "otu_table", cols="sa1")
# sample.metacoder$data$otu_abund$sa1 <- sample.metacoder$data$otu_abund$sa1
# sample.metacoder$data$otu_abund$sa1 <- round(sample.metacoder$data$otu_abund$sa1, digits = 3)
#
# set.seed(1)
#
# heat_tree(sample.metacoder,
#           node_size = sample.metacoder$data$otu_abund$sa1,
#           node_color = sample.metacoder$data$otu_abund$sa1,
#           make_node_legend = TRUE,
#           make_edge_legend = TRUE,
#           node_label = taxon_names,
#           node_color_axis_label = "% Taxonómica",
#           layout = "davidson-harel",
#           initial_layout = "reingold-tilford",
#           background= "transparent",
#           output_file = outputFileTree)
#
# print("TREE GRAPH DONE")

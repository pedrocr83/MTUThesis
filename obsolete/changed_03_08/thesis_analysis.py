...
def run_OPTICS(csv_file_path, results_df, distance=default_distance_metric):
    ...
    threshold = np.quantile(scores, 1 - outlier_value)
    outlier_index = np.where(scores >= threshold)
    outlier_values = temp_df.iloc[outlier_index]
    outlier_samples = outlier_values.index.values.tolist()
...
def Main():
    ...
    parser.add_argument("-d", "--distance_measure", help="DISTANCE USED IN R SCRIPT (gunifrac, unifrac, wunifrac, va-wunifrac)", required=True)

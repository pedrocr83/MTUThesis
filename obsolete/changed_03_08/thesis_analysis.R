...
phyloseq_results <- function(phyloseq_obj, taxa_rank, transformation, class_variable, phasetxt)
{
    ...
    result1 <- plot_ly(phyloseq_obj.mds, x=~MDS1,y=~MDS2,z=~MDS3) %>% add_markers(color=phyloseq_obj.mds[[class_variable]], colors = c("darkgreen", "red"), size=3)
    ...
    result2 <- plot_ly(phyloseq_obj.tsne, x=~tSNE1,y=~tSNE2,z=~tSNE3) %>% add_markers(color=~variable, colors = c("darkgreen", "red"), size=3)
...
for (var in vars_list)
    {
        ...
        heat_tree(class1.metacoder, node_label = taxon_names,
                  ...
                  node_color_interval = c(-2, 2), node_color_range = cccolors,
...
if (phase1 == TRUE) {
    ...
    phyloseq_results(study.phase1, taxa_rank, transformation, class_variable, 'phase1')
    ...
    study.class1 <- phyloseq(otu_table(study.class1, taxa_are_rows = T), sample_data(study.class1.mapping), tax_table(as.matrix(study.taxonomy)), study.tree)
    ...
    study.class2 <- phyloseq(otu_table(study.class2, taxa_are_rows = T), sample_data(study.class2.mapping), tax_table(as.matrix(study.taxonomy)), study.tree)
    ...
    for(i in 1:nrow(col_df)) {
        ...
        phyloseq_results(study.phase1, taxa_rank, transformation, class_variable, paste(file_name, 'phase2', sep='_'))

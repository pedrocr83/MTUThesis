...
phyloseq_distance <- function(phyloseq_obj, distance_measure)
{
    if (tolower(distance_measure) == 'unifrac') {
        distance_obj <- phyloseq::distance(phyloseq_obj, method = 'unifrac')
    } else if (tolower(distance_measure) == 'wunifrac') {
        distance_obj <- phyloseq::distance(phyloseq_obj, method = 'wunifrac')
    } else if  (tolower(distance_measure) == 'gunifrac') {
        distance_obj <- microViz::dist_get(microViz::dist_calc(phyloseq_obj, dist='gunifrac'))
    }
    return(distance_obj)
}
    ...
    for (var in vars_list)
        {
            ...
            class1_tree_obj_path <- file.path(temp_files, paste(paste(class1_str, var, sep='_'), 'RDS', sep='.'))
            class2_tree_obj_path <- file.path(temp_files, paste(paste(class2_str, var, sep='_'), 'RDS', sep='.'))

            set.seed(39)

            class1.heat_tree <- heat_tree(class1.metacoder, node_label = taxon_names,
            ...
            class2.heat_tree <- heat_tree(class2.metacoder, node_label = taxon_names,
                      node_size = n_obs, node_color = mean_diff,
                      node_color_interval = c(-20, 20), node_color_range = cccolors,
                      make_node_legend = FALSE, make_edge_legend = FALSE,
                      layout = "davidson-harel", initial_layout = "reingold-tilford",
            ...
            saveRDS(class1.heat_tree, file = class1_tree_obj_path)
            saveRDS(class2.heat_tree, file = class2_tree_obj_path)
        }
}
...
glue("TRANSFORMATION: {transformation}")
glue("UNIFRAC DISTANCE: {distance_measure}")
...
if (phase1 == TRUE) {
    ...
    # CLASS 1
    study.class1.distances <- phyloseq_distance(study.class1, distance_measure)
    ...
    # CLASS 2
    study.class2.distances <- phyloseq_distance(study.class2, distance_measure)
    ...
    study.phase1 <- phyloseq_aggregation(study, taxa_rank)
    study.phase1 <- phyloseq_transformation(study.phase1, 'relative')

    cat('\nPERMANOVA TESTING PHASE 1 \n')
